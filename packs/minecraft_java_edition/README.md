...
| `app_mods_file` | Path to file with Mod URLs (e.g.: `/extras/mods.txt`) | `` |
...
| `app_spawn_protection` | Sets area that non-ops cannot alter (in blocks). | `0` |
...
| `app_world` | URL to Minecraft World ZIP archive. | `` |
...
| `nomad_group_ports` | Port Configuration for the Group. | `map[main:map[check_interval:30s check_timeout:15s host_network:<nil> name:minecraft_main path:<nil> port:25565 type:tcp] rcon:map[check_interval:30s check_timeout:15s host_network:<nil> name:minecraft_rcon path:<nil> port:25575 type:tcp]]` |
...
| `nomad_group_volumes` | Volumes for the Group. | `map[minecraft_data:map[destination:/data name:minecraft_data read_only:false type:host] minecraft_extras:map[destination:/extras name:minecraft_extras read_only:false type:host] minecraft_worlds:map[destination:/worlds name:minecraft_worlds read_only:false type:host]]` |
...
| `nomad_task_image` | Content Address to use for the Container Image for the Task. | `map[digest:sha256:722dde03948a7979681221b31b99d08608e082e8d76c9b65b5dbb20791278da6 image:minecraft-server namespace:itzg registry:index.docker.io tag:2023.6.4-java20-alpine]` |
