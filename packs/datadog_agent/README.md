...
| ---- | ----------- | ------- |
| `app_dd_ac_exclude` | List of Containers to exclude from Metrics and Auto Discovery. | `[]` |
| `app_dd_ac_include` | List of Containers to include in Metrics and Auto Discovery. | `[]` |
| `app_dd_ad_config_poll_interval` | Interval (in seconds) to check for new Auto Discovery configurations. | `10` |
| `app_dd_additional_checksd` | Path to the directory containing additional Python checks. | `` |
...
| `app_dd_apm_dd_url` | Hostname of the APM Server to use. | `` |
| `app_dd_apm_enabled` | Toggle to enable the APM Agent. | `true` |
| `app_dd_apm_env` | Environment Tag for APM. | `` |
| `app_dd_apm_error_tps` | Target Error Trace chunks to receive per second. | `10` |
| `app_dd_apm_features` | List of Beta APM Features to enable. | `[]` |
| `app_dd_apm_ignore_resources` | List of Exclusions of Regular Expressions to disable certain traces based on their resource name. | `[]` |
| `app_dd_apm_log_file` | Path to APM Agent Log file. | `` |
...
| `app_dd_apm_max_cpu_percent` | CPU Percentage the Agent is allowed to use. | `0` |
| `app_dd_apm_max_eps` | Maximum Events per second to sample. | `200` |
| `app_dd_apm_max_memory` | Memory the Agent is allowed to use. | `0` |
...
| `app_dd_apm_receiver_socket` | Path to the APM Receiver Socket. | `` |
| `app_dd_apm_replace_tags` | List of Rule Objects to replace or remove certain Resources or Tags containing sensitive information. | `&{}` |
| `app_dd_auto_exit_noprocess_enabled` | Toggle to enable the `noprocess` method. | `false` |
| `app_dd_auto_exit_noprocess_excluded_processes` | List of Regular Expressions to exclude extra processes with. | `[]` |
...
| `app_dd_autoconf_template_dir` | Path to the directory containing Autoconfig configuration files. | `` |
| `app_dd_autoconfig_exclude_features` | List of features to exclude from Autoconfig. | `[]` |
| `app_dd_autoconfig_include_features` | Activate Autoconfig features as if they were discovered by autodiscovery. | `[]` |
...
| `app_dd_confd_path` | Path to the directory containing `checks.d` configuration files. | `` |
...
| `app_dd_container_env_as_tags` | List of Container Runtime Environment Variables to extract and set as Metric Tags. | `[]` |
| `app_dd_container_labels_as_tags` | Key-Value Pairs of Container Labels to be set as Tags. | `[]` |
...
| `app_dd_docker_env_as_tags` | List of Docker Environment Variables to extract and set as Metric Tags. | `[]` |
| `app_dd_docker_labels_as_tags` | Key-Value Pairs of Docker Labels to be set as Tags. | `[]` |
...
| `app_dd_dogstatsd_so_rcvbuf` | Size (in bytes) of the socket receive buffer for DogStatsD. | `0` |
| `app_dd_dogstatsd_socket` | Path to the DogStatsD Socket. | `` |
...
| `app_dd_dogstatsd_tags` | Additional Tags to append to all Metrics, Events, and Service Checks received by DogStatsD. | `[]` |
...
| `app_dd_exclude_ec2_tags` | AWS EC2 tags to exclude from being collected as Host Tags. | `[]` |
...
| `app_dd_extra_config_providers` | List of additional Config Providers to enable. | `[]` |
| `app_dd_extra_listeners` | List of additional Listeners to enable. | `[]` |
| `app_dd_flare_stripped_keys` | Additional sensitive keys that should be scrubbed before sending an Agent Flare. | `[]` |
...
| `app_dd_histogram_copy_to_distribution_prefix` | Prefix to add to Distribution Metrics. | `` |
...
| `app_dd_jmx_custom_jars` | List of custom JARs to load for JMXFetch. | `[]` |
| `app_dd_jmx_log_file` | Path to JMXFetch Log file. | `` |
...
| `app_dd_logs_config_dd_url` | Hostname of the Logs Server to use. | `` |
...
| `app_dd_logs_config_processing_rules` | List of Processing Rules to apply to all Logs | `[]` |
...
| `app_dd_observability_pipelines_worker_logs_url` | URL endpoint for the Observability Pipelines Worker to send Logs to. | `` |
...
| `app_dd_observability_pipelines_worker_metrics_url` | URL endpoint for the Observability Pipelines Worker to send Metrics to. | `` |
...
| `app_dd_observability_pipelines_worker_traces_url` | URL endpoint for the Observability Pipelines Worker to send Traces to. | `` |
...
| `app_dd_otlp_config_traces_span_name_remappings` | List of Span Name Remappings to apply to OpenTelemetry Span Names. | `[]` |
| `app_dd_process_config_blacklist_patterns` | List of Regular Expressions to exclude extra processes with. | `[]` |
...
| `app_dd_process_config_log_file` | Path to Process Agent Log file. | `` |
| `app_dd_process_config_max_per_message` | Maximum Number of Processes or Containers to send per message. | `100` |
| `app_dd_process_config_process_queue_bytes` | Amount of Data (in bytes) to buffer in memory when a POST to the Intake Server fails. | `6e+07` |
...
| `app_dd_procfs_path` | Path to the `procfs` file system. | `` |
...
| `app_dd_secret_backend_arguments` | List of arguments to give to the Secret Backend Command at each run. | `[]` |
| `app_dd_secret_backend_command` | Path to a Command to execute to fetch Secrets. | `` |
| `app_dd_secret_backend_output_max_size` | Size (in bytes) of the buffer used to store the Secrets Backend Command's response. Applies to `stdout` and `stderr`. | `1.048576e+06` |
...
| `app_dd_statsd_metric_namespace` | Namespace for all StatsD Metrics for this Agent. | `` |
| `app_dd_syslog_key` | Path to PEM key for TLS-enabled Syslog. | `` |
| `app_dd_syslog_pem` | Path to PEM certificate for TLS-enabled Syslog. | `` |
...
| `app_dd_syslog_uri` | Custom Remote Syslog URI. | `` |
| `app_dd_tracemalloc_debug` | Toggle to enable debugging with `tracemalloc` for Python checks. | `false` |
| `app_dd_tracemalloc_exclude` | List of Python checks to disable `tracemalloc` for, when `tracemalloc_debug` is true. | `[]` |
| `app_dd_tracemalloc_include` | List of Python checks to enable `tracemalloc` for, when `tracemalloc_debug` is true. | `[]` |
| `app_dd_url` | Hostname of the Intake Server to use. | `https://app.datadoghq.com` |
| `app_dd_use_proxy_for_cloud_metadata` | Toggle to remove CSP IPs from the transport's `no_proxy` list. | `&{}` |
...
| `nomad_group_ports` | Port Configuration for the Group. | `map[apm_expvar:map[check_interval:30s check_timeout:15s host_network:<nil> name:datadog_agent_apm_expvar path:<nil> port:5012 protocol:tcp type:tcp] apm_receiver:map[check_interval:30s check_timeout:15s host_network:<nil> name:datadog_agent_apm_receiver path:<nil> port:8126 protocol:tcp type:tcp] config:map[check_interval:30s check_timeout:15s host_network:<nil> name:datadog_agent_config path:<nil> port:6162 protocol:tcp type:tcp] debug:map[check_interval:30s check_timeout:15s host_network:<nil> name:datadog_agent_debug path:<nil> port:6062 protocol:tcp type:tcp] dogstatsd:map[check_interval:30s check_timeout:15s host_network:<nil> name:datadog_agent_dogstatsd path:<nil> port:8125 protocol:tcp type:tcp] expvar:map[check_interval:30s check_timeout:15s host_network:<nil> name:datadog_agent_expvar path:<nil> port:5000 protocol:tcp type:tcp] gui:map[check_interval:30s check_timeout:15s host_network:<nil> name:datadog_agent_gui path:/ port:5002 protocol:http type:http] healthcheck:map[check_interval:30s check_timeout:15s host_network:<nil> name:datadog_agent_healthcheck path:/healthcheck port:5555 protocol:http type:http] ipc:map[check_interval:30s check_timeout:15s host_network:<nil> name:datadog_agent_ipc path:/agent/status port:5001 protocol:https type:http]]` |
